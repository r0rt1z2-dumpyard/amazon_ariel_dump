...
if [[ "$evt_build" == "$ARIEL_EVT1_BOARD_ID"* || "$evt_build" == "$ARIEL_EVT1_1_BOARD_ID"* ||
   ...
   echo "This is not an Ariel EVT1 nor Aston proto." >> $LOG
   if [[ "$PARTITION_KB" == *"EKKB"* && "$PARTITION_KB" == *"PLAYREADY_BGROUPCERT"* &&
	 ...
	 echo "All key blocks exist." >> $LOG
   else
	 echo "Some key blocks are missing, so we are doing re-provisioning." >> $LOG
	 $DRMDBGTOOL e f
	 $DRMDBGTOOL e o
...
if [[ "$PARTITION_KB" == *"LEK"* ]]; then
   ...
   echo "LEK is not provisioned, provisioning KB_PM now." >> $LOG
   if [[ -f $KB_PM ]]; then
      mv $KB_PM /data/key_provisioning/FORCE_KB_PM
      $DRMDBGTOOL i KB_PM
      echo "Done provisioning KB_PM." >> $LOG
   else
      echo "KB_PM doesn't exist, failed provisioning LEK." >> $LOG
   fi
fi
